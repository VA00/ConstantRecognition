// constant_gpu.cu
// Complete optimized GPU implementation for constant recognition
// Compile: nvcc -O3 -arch=sm_75 constant_gpu.cu -o constant_gpu
...
// ============================================================================
// Device constants in constant memory (fastest access)
// ============================================================================

__constant__ double d_const_values[N_CONST] = {
    3.14159265358979323846264338327950288,   // 0: PI
    2.71828182845904523536028747135266250,   // 1: EULER
   -1.0,                                      // 2: NEG
    1.61803398874989484820458683436563812,   // 3: GOLDENRATIO
    1.0,                                      // 4: ONE
    2.0,                                      // 5: TWO
    3.0,                                      // 6: THREE
    4.0,                                      // 7: FOUR
...
// ============================================================================
// Direct evaluation kernel - no string building
// ============================================================================

__device__ __forceinline__ double apply_unary(int op, double x)
{
    switch(op) {
        case 0:  return log(x);
        case 1:  return exp(x);
        case 2:  return 1.0 / x;
        case 3:  return tgamma(x);
        case 4:  return sqrt(x);
        case 5:  return x * x;
        case 6:  return sin(x);
        case 7:  return asin(x);
        case 8:  return cos(x);
        case 9:  return acos(x);
        case 10: return tan(x);
        case 11: return atan(x);
        case 12: return sinh(x);
        case 13: return asinh(x);
        case 14: return cosh(x);
        case 15: return acosh(x);
        case 16: return tanh(x);
        case 17: return atanh(x);
        default: return nan("");
    }
}

__device__ __forceinline__ double apply_binary(int op, double a, double b)
{
    switch(op) {
        case 0: return a + b;
        case 1: return a * b;
...
__device__ double evaluate_form_direct(
...
// ============================================================================
// Main search kernel - processes one form, all its instantiations
// ============================================================================
...
__global__ void search_form_kernel(
    ...
    unsigned long long total,
    unsigned long long offset,          // For chunked execution
    double targetX,
    double Delta_z,
...
)
{
    ...
    // Evaluate directly
    double computedX = evaluate_form_direct(s_ternary, slots, K);
    ...
    double rel_err;
    if (targetX == 0.0)
        rel_err = fabs(computedX);
    else
        rel_err = fabs(computedX / targetX - 1.0);
    
    // Load current best for comparison
    double current_best = best_result->error;
    
    // Only attempt atomic if we might improve
    if (rel_err < current_best) {
        // Atomic compare-and-swap for double
        unsigned long long* addr = (unsigned long long*)&best_result->error;
        unsigned long long old = __double_as_longlong(current_best);
        unsigned long long assumed;
        
        do {
            assumed = old;
            if (__longlong_as_double(assumed) <= rel_err) break;
            old = atomicCAS(addr, assumed, __double_as_longlong(rel_err));
        } while (assumed != old);
        
        // If we successfully updated, store our result info
        if (__longlong_as_double(old) > rel_err) {
            best_result->idx = global_idx;
            best_result->form_id = form_id;
            best_result->K = K;
            
            // Check for "exact" match
            if (rel_err <= EPS_MAX * DBL_EPSILON) {
                atomicExch(found_flag, 1);
            }
            
            // Also check compression criterion
            if (Delta_z > 0.0) {
                double abs_err = fabs(computedX - targetX);
                if (abs_err <= 2.0 * Delta_z) {
...
// ============================================================================
// Host: Evaluate on CPU for verification
// ============================================================================

double evaluate_cpu(const char* amino, int K)
{
...
int main(int argc, char** argv)
{
    ...
    printf("=== GPU Constant Recognition ===\n");
    printf("Target: %.17g\n", targetX);
    printf("Delta_z: %.17g\n", Delta_z);
    printf("Max K: %d\n", MaxCodeLength);
    ...
    int max_forms = 100000;  // Should be enough for K <= 12
    ...
    if (h_best.form_id >= 0 && h_best.form_id < total_forms) {
        ...
        computed_value = evaluate_cpu(amino, h_best.K);
    }
    
    // Print results
    printf("\n");
    printf("=== Results ===\n");
    printf("Status: %s\n", h_found ? "SUCCESS (exact match)" : "BEST FOUND");
    printf("Target:   %.17g\n", targetX);
    printf("Computed: %.17g\n", computed_value);
    printf("Relative error: %.17e\n", h_best.error);
    printf("Error in eps:   %.6f × DBL_EPSILON\n", h_best.error / DBL_EPSILON);
    printf("\n");
